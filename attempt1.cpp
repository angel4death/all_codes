      ...
      if(t)
	{
	  ...
	  for(int p=0;p<6;++p)
	    res_col[k][p]=ccount[p];
          if(pre[k][res_mask[k]][ccount[0]][ccount[1]][ccount[2]][ccount[3]][ccount[4]]==-1)
            continue;
             dfs(k+1);
            if(found==1)
	      ...
	      {
		pre[k][res_mask[k]][res_col[k][0]][res_col[k][1]][res_col[k][2]][res_col[k][3]][res_col[k][4]]=-1;
	          continue;
	      }
	}
